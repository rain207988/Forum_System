...
class MessageControllerMysqlIntegrationTest {
    ...
    void shouldSendAndQueryMessagesViaHttp() throws Exception {
        String senderToken = loginToken(USER_ONE_ID);
        String receiverToken = loginToken(USER_TWO_ID);
        String sendContent = "http-send-" + System.nanoTime();

        mockMvc.perform(post("/message/send")
                        .header("Authorization", bearerToken(senderToken))
        ...
        mockMvc.perform(get("/message/getUnreadCount")
                        .header("Authorization", bearerToken(receiverToken)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.code", is(0)))
    ...
    private String loginToken(Long userId) {
        return jwtAuthenticationService.generateToken(userService.selectById(userId));
    }
